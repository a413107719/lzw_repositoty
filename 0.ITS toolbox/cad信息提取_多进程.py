...
def extract_kindfeatures(feature_kind, cad, cad_folder, gdb_path, max_row, sheet):
    ...
    for name in range(2, max_row + 1):
        ...
        # print('name:' + str(sheetvalue))
        if sheetvalue is not None:
            # extract_features(sheet, cad, name, sheetvalue, time2, dwg_feature, gdb_path)
            pool.apply_async(extract_features, (sheet, cad, name, sheetvalue, time2, dwg_feature, gdb_path))
    pool.close()
    pool.join()
